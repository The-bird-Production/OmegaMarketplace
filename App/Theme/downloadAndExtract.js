...
async function downloadAndExtractValidatedThemes() {
  try {
    ...
    const artifacts = response.data.artifacts
      .filter((artifact) => artifact.name === "validated-themes")
      .sort((a, b) => new Date(b.created_at) - new Date(a.created_at)); // Sort by creation date (latest first)

    if (artifacts.length === 0) {
      console.log("No validated theme artifacts found.");
      return;
    }
    ...
    await extractZip(zipPath, VALIDATED_THEME_DIR);
...
module.exports = downloadAndExtractValidatedThemes;
